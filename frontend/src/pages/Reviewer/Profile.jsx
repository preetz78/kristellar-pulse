...
function ReviewerProfile() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch Profile
        const profileRes = await fetch(`${apiConfig.API_BASE_URL}/api/reviewer/profile`, {
  ...
  const handleChangePassword = async () => {
    ...
    try {
      ...
      const res = await fetch(`${apiConfig.API_BASE_URL}/api/reviewer/change-password`, {
        method: "PUT",
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json"
  ...
  if (!reviewerData) return <div className="p-12 text-center">No profile data found</div>;
      ...
      <div className="relative bg-gradient-to-r from-blue-700 via-blue-600 to-blue-500 rounded-2xl p-6 text-white shadow-xl">
        <div className="flex flex-col md:flex-row items-center gap-6">
          ...
          <div className="w-24 h-24 rounded-2xl bg-white text-blue-600 flex items-center justify-center text-4xl font-bold shadow-lg overflow-hidden">
            {reviewerData.profile_picture ? (
              <img 
                src={`${apiConfig.API_BASE_URL}${reviewerData.profile_picture}`} 
                alt="Profile" 
                className="w-full h-full object-cover"
              />
            ) : (
              reviewerData.name?.charAt(0) || "P"
            )}
          </div>
        ...
        <div className="lg:col-span-2 bg-white/90 backdrop-blur rounded-2xl p-6 shadow-lg">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-gray-700">
            ...
            <InfoRow
              icon={<Calendar />}
              label="Joined"
              value={reviewerData.created_at ? new Date(reviewerData.created_at).toDateString() : ""}
            />
            <InfoRow icon={<Shield />} label="Role" value="Reviewer" />
          </div>
